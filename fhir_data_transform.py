...
def explore_resource_type(resources: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Explore a specific resource type and summarize its fields and values.

    Args:
        resources (List[Dict[str, Any]]): List of resources of the same type.
    Returns:
        Dict[str, Any]: Field presence, unique values, and stats.
    """
    ...
    for resource in resources:
        ...
        for key, value in resource.items():
            ...
            elif key == "valueQuantity" and isinstance(value, dict):
                field_values["valueQuantity.value"].add(value.get("value"))
                field_values["valueQuantity.unit"].add(value.get("unit"))
            elif key == "code" and isinstance(value, dict):
                field_values["code.text"].add(value.get("text"))

    stats = {}
    if resources[0]["resourceType"] == "Observation":
        values = [
            r.get("valueQuantity", {}).get("value")
            for r in resources
            if isinstance(r.get("valueQuantity", {}).get("value"), (int, float))
        ]
        if values:
            stats = {
                "min": min(values),
    ...
    return {
        "total": len(resources),
        "field_presence": {k: v / len(resources) * 100 for k, v in field_presence.items()},
        "unique_values": {k: list(v)[:10] for k, v in field_values.items()},
...
def generate_exploration_report(exploration: Dict[str, Any], output_path: str) -> None:
    """
    Generate a text report from the exploration summary.

    Args:
        exploration (Dict[str, Any]): Exploration results.
    ...
    with open(output_path, "w") as f:
        ...
        for resource_type, details in exploration["details"].items():
            ...
            for field, pct in details["field_presence"].items():
                f.write(f"    - {field}: {pct:.2f}%\n")
            f.write("  Unique Values (sample):\n")
            for field, values in details["unique_values"].items():
...
def preprocess_patients(patients: Dict[str, Dict]) -> Dict[str, Dict]:
    ...
    for pid, data in patients.items():
        ...
        try:
            data["birthDate"] = pd.to_datetime(data["birthDate"]).strftime("%Y-%m-%d")
        except (ValueError, TypeError):
            data["birthDate"] = "unknown"
    return patients
'''
...
def visualize_biosignals(observations: List[Dict[str, Any]], output_dir: str, filename: str, title: str) -> None:
    """
    Visualize time series of biosignals for all patients, with one subplot per biosignal code.

    Args:
        observations: List of observation dictionaries with patient_id, code, value, unit, timestamp.
        output_dir: Directory to save the plot.
        filename: Name of the output file (e.g., 'raw_observations.png').
        title: Title of the plot.
    """
    if not observations:
        logger.warning(f"No observations to visualize for {filename}")
        return
    ...
    # Get unique biosignal codes
    codes = df['code'].unique()
    if not codes:
        logger.warning(f"No biosignal codes found for {filename}")
        return

    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    
    # Create figure with subplots (one per code)
    fig, axes = plt.subplots(len(codes), 1, figsize=(12, 4 * len(codes)), sharex=True)
    if len(codes) == 1:
        axes = [axes]  # Ensure axes is a list for single subplot
    
    # Plot each biosignal code
    for ax, code in zip(axes, codes):
        ...
        for patient_id in code_df['patient_id'].unique():
            patient_df = code_df[code_df['patient_id'] == patient_id]
            patient_df = patient_df.sort_values('timestamp')
            ax.plot(
                patient_df['timestamp'],
                patient_df['value'],
                label=f"Patient {patient_id[:8]}",
                marker='o'
            )
        
        ax.set_title(f"{code} ({code_df['unit'].iloc[0] if not code_df['unit'].empty else 'unknown'})")
        ax.set_ylabel('Value')
        ax.legend()
        ax.grid(True)
    
    # Set x-axis label and rotate dates for readability
    if codes:
    ...
    plt.close()
    logger.info(f"Saved biosignals plot to {output_path}")
'''
def process_fhir_bundle(fhir_json: str, output_dir: str = "fhir_analysis") -> Dict[str, Any]:
    ...
    logging.info(f"Sample observations after preprocessing: {observations}")
    # Visualize biosignals
    visualize_biosignals_interactive(observations, output_dir)

    patients = preprocess_patients(patients)
    logging.info(f"Sample patients after preprocessing: {patients}")
    df = pd.DataFrame(observations)
    logging.info(f"Sample observations after preprocessing: {df}")
    if df.empty:
        ...
        with ProgressBar():
            ...
            if "SLEEP_DURATION" in unique_codes:  # Update with actual code
